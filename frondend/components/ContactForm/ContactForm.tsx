...
const ContactForm = () => {
    ...
    const [nameInfo, setNameInfo] = useState<InputInfo>({
        valid: false,
        value: "",
    });
    const [subjectInfo, setSubjectInfo] = useState<InputInfo>({
        valid: false,
        value: "",
    });
    const [messageInfo, setMessageInfo] = useState<InputInfo>({
        valid: false,
        value: "",
    });
    ...
    return (
        <form action="" className={styleForm}>
            <Input
                type={InputType.name}
                callbackValidate={(phoneInfo: InputInfo) =>
                    setNameInfo(phoneInfo)
                }
            />
            <Input
                type={InputType.email}
                callbackValidate={(emailInfo: InputInfo) =>
                    setEmailInfo(emailInfo)
                }
            />
            <Input
                type={InputType.subject}
                wrapperClass={styles.subject}
                callbackValidate={(subjectInfo: InputInfo) =>
                    setSubjectInfo(subjectInfo)
                }
            />
            <Input
                type={InputType.message}
                wrapperClass={styles.message}
                callbackValidate={(messageInfo: InputInfo) =>
